...
class Telegram
{
    ...
    public function Phone()
    {
        $type = $this->getUpdateType();
        if ($type == self::CALLBACK_QUERY) {
            return @$this->data['callback_query']['from']['phone'];
        }
        if ($type == self::CHANNEL_POST) {
            return @$this->data['channel_post']['from']['phone'];
        }
        if ($type == self::EDITED_MESSAGE) {
            return @$this->data['edited_message']['from']['phone'];
        }

        return @$this->data['message']['from']['phone'];
    }
