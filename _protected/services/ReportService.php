...
class ReportService
{
    ...
    public static function queryFactorys($contract, $year)
    {
        ...
        $inv2 = Yii::$app->db->createCommand($query2)->queryAll();
        foreach($inv2 as $row2){
            $contracts[$row2['contract']]['invoice_no'][] = $row2['invoice_no'];
            $query3 = "SELECT waybill_id FROM `fg_invoice_waybill` where fg_invoice_id = '".$row2['id']."'";
            $inv3 = Yii::$app->db->createCommand($query3)->queryAll();
            foreach($inv3 as $row3){
  ...
  // fakt bo'yicha qilingan otchot update qilindi
	...
	*/
    ...
    public function productionPlanFactDaily($yesterDay)
    {
        ...
        foreach($parts as $key =>  $part){
            $yesterDayFact = Yii::$app->db->createCommand($queryFact,[':part_id' => $part['part_id'], ':line' => $part['line'], ':interval1'=> $yesterDay.' 08:00', ':interval2'=>$today.' 07:59'])->queryScalar();
            ...
            $yesterDayFactShift1 = Yii::$app->db->createCommand($queryFact,[':part_id' => $part['part_id'], ':line' => $part['line'], ':interval1'=>$yesterDay.' 08:00', ':interval2'=>$yesterDay.' 19:59'])->queryScalar();
            ...
            $yesterDayFactShift2 = Yii::$app->db->createCommand($queryFact,[':part_id' => $part['part_id'], ':line' => $part['line'], ':interval1'=>$yesterDay.' 20:00', ':interval2'=>$today.' 07:59'])->queryScalar();
            ...
            for($i=$date22; $i <= $endDate; $i = date("Y-m-d", strtotime($i.'+1 day'))){
                ...
                $planShift1 = Yii::$app->db->createCommand($queryPlan,[':date' => $i, ':line' => $part['line'], ':part_id' => $part['part_id'], ':shift' => 1])->queryScalar();
                $planShift2 = Yii::$app->db->createCommand($queryPlan,[':date' => $i, ':line' => $part['line'], ':part_id' => $part['part_id'], ':shift' => 2])->queryScalar();
                $fact = Yii::$app->db->createCommand($queryFact,[':part_id' => $part['part_id'], ':line' => $part['line'], ':interval1'=> $i.' 08:00', ':interval2'=>$day1.' 07:59'])->queryScalar();
    ...
    public function getWeeklyRequirementShorts($part_id = null)
    {
      ...
      if($part_id){
        ...
        foreach($generalPartList as $key => $part){
            $currentWeek = [];
            $currentWeek = Yii::$app->db->createCommand($query, [':date1' => $fromCurrentMonth, ':date2' => $toCurrentMonth, ':part_id' => $part['part_id']])->queryAll();
            if(!empty($currentWeek)){
              $currentWeek = array_sum(array_column($currentWeek, 'qty'));
            }
            // return $currentWeek;
            ...
            $generalPartList[$key]['currentWeekBalance']  = $generalPartList[$key]['stock'] - $generalPartList[$key]['current_week']    +   $part['arrived_qty'];
     ...
     */
    //monthly requirement shorts
    public function getDailyRequirementShorts($part_id = null)
    {
        

        $fromCurrentMonth = date("Y-m").'-01';
        $toCurrentMonth = date("Y-m-t");
        $query = "SELECT sum(pp.target_qty * psi.usage_qty / ps.amount) as qty  FROM production_daily_plan pp 
            INNER JOIN product_specification ps on ps.part_id = pp.part_id
            INNER JOIN product_specification_item psi on psi.product_specification_id = ps.id
            where pp.production_date between :date1 and :date2
            and psi.part_id = :part_id and ps.status = 1
      ...
      if($part_id){
        ...
        foreach($generalPartList as $key => $part){
            $currentDay = [];
            $currentDay = Yii::$app->db->createCommand($query, [':date1' => $fromCurrentMonth, ':date2' => $toCurrentMonth, ':part_id' => $part['part_id']])->queryAll();
            if(!empty($currentDay)){
              $currentDay = array_sum(array_column($currentDay, 'qty'));
            }
            // return $currentDay;
            $generalPartList[$key]['current_day'] = $currentDay?(round($currentDay)):0;
            ...
            $generalPartList[$key]['currentDailyBalance']  = $generalPartList[$key]['stock'] - $generalPartList[$key]['current_day']    +   $part['arrived_qty'];
            $generalPartList[$key]['averageUsage']        = Part::findOne($part['part_id'])->averageUsage?:0;
            $generalPartList[$key]['vputis'] = [];
            $vputis = Yii::$app->db->createCommand($invoice_detail_query, [':part_id'=>$part['part_id']])->queryAll();
            $arr = [];
        ...
        $columns = array_column($generalPartList, 'currentDailyBalance');
        array_multisort($columns, SORT_ASC, $generalPartList);
        // vd($generalPartList);
      return $generalPartList;
    }
    public function getAdditionalDailyRequirementShort($partId, $qty)
    {
        $query = 'SELECT p.part_name, p.part_no, p.part_color, p.remark  , (pmp.target_qty*psi.usage_qty)/ps.amount  as amount, pmp.target_qty as qty FROM `product_specification_item`  psi
          inner JOIN product_specification ps on psi.product_specification_id=ps.id
          inner JOIN production_daily_plan pmp on ps.part_id=pmp.part_id
          inner join part p on pmp.part_id = p.id
          WHERE psi.part_id=:partId and ps.status=1
          and pmp.production_date between :date1 and :date2
        	';
