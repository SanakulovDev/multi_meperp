...
use app\models\Part;
use app\models\ProductionWeeklyPlan;
use app\services\TelegramService;
use app\models\ProductSpecification;
use app\models\ProductionPlanComment;
use app\models\ProductionWeeklyPlanSearch;
...
class ProductionWeeklyPlanController extends Controller {

  /**
   * Lists all ProductionWeeklyPlan models.
   *
   * @return mixed
   */
  public function actionIndex() {
    $searchModel = new ProductionWeeklyPlanSearch();
  ...
  public function actionValidate($id = null) {
    $model = $id === null ? new ProductionWeeklyPlan() : ProductionWeeklyPlan::findOne($id);
  ...
  /**
   * Creates a new ProductionWeeklyPlan model.
   * If creation is successful, the browser will be redirected to the 'view' page.
   *
   * @return mixed
   */
  public function actionCreate() {
    $models	= [new ProductionWeeklyPlan];
    if(Yii::$app->getRequest()->isAjax) {
        
      if(Yii::$app->request->post()) {
        $models = Model::createMultiple(ProductionWeeklyPlan::classname());
        ...
        if($valid){
          ...
          try{
                $flag = true;
                  ...
                  //TelegramService::ProductionWeeklyPlan($tg);
              	}
            if ($flag) {
                $transaction->commit();
                Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        return $this->renderAjax('_form2', [
          'models' 		=> (empty($models))?[new ProductionWeeklyPlan]:$models
        ]);
      }
    } else {
  ...
  /**
   * Comment an existing ProductionWeeklyPlan definition.
  ...
  /**
   * Deletes an existing ProductionWeeklyPlan model.
  ...
  public function actionDeleteAll() {
    Yii::$app->response->format = Response::FORMAT_JSON;
    //table truncate
    $table = ProductionWeeklyPlan::tableName();
  ...
  /**
   * Finds the ProductionWeeklyPlan model based on its primary key value.
   * If the model is not found, a 404 HTTP exception will be thrown.
   *
   * @param int $id
   *
   * @return ProductionWeeklyPlan the loaded model
   * @throws NotFoundHttpException if the model cannot be found
   */
  protected function findModel($id) {
    if(($model = ProductionWeeklyPlan::findOne($id)) !== null) {
      return $model;
    }
