...
$this->title = Yii::t('app', 'Shop consumption');
				...
				<div class="box-body table-responsive no-padding">
					<table class="table table-hover table-striped table-order-600">
						<tbody>
							...
							<? foreach ($last_consumption_docs as $cons) { ?>

								<tr>
									<td>
										<? if (Yii::$app->user->can('document-delete-shop-consumption')) { ?>
											<?=
												Html::a(null, Url::to(['delete-shop-consumption', 'id' => $cons->id]), [
									...
									<td><?= $cons->serial_number ?></td>
									<td><?= $cons->docnum ?></td>
									<td><?= $cons->documentDetails[0]->part->partinfo ?></td>
									<td class="text-center"><?= number_format($cons->documentDetails[0]->qty, 0, '', '') ?></td>
									<td><?= $cons->createdAtFormatted ?></td>
									<td><?= $cons->createdBy->fullname ?></td>
								</tr>
