...
<div class="content-index">
	<div class="row">
		<div class="col-lg-6 col-lg-offset-3">
			<div>
				<a href="<?= Url::toRoute(['report/material-stock', 'state' => Part::STATE_RAW]) ?>" class="btn <?=(!isset($_GET['state']) or $_GET['state'] == Part::STATE_RAW) ? 'btn-success': 'btn-primary'?>">Базовое сырье</a>
				<a href="<?= Url::toRoute(['report/material-stock', 'state' => Part::STATE_FINISHED]) ?>" class="btn <?=(isset($_GET['state']) and $_GET['state'] == Part::STATE_FINISHED) ? 'btn-success': 'btn-primary'?>">Готовый продукт</a>
			</div>
			<div class="mini_header">млн Сум</div>
			<table class="tbl-plan tbl-first" id="fix_table">
				<tr class="tr_head">
					<td rowspan="2">№</td>
					<td rowspan="2">Полимерные компаунды</td>
				...
				<?php foreach ($data['data'] as $row) { ?>
					...
					<tr class="tr_item <?= $classOdd ?>">
						...
						<td><a href="<?= Url::toRoute(['report/material-stock-by-type', 'state' => $_GET['state'] ?? 0, 'type' => $row['type_id']]) ?>"><?= $row['type_name'] ?></a> </td>
