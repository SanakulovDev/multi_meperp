...
<div class="req-index">
	<div class="row">
		<div id="div_fix_table" class="col-lg-10 col-lg-offset-1">
			...
			<table id="table_summary" class="table table-bordered table-doh">
				<tbody>
					<tr>
            ...
            <?}?>
						...
						<td class="text-center" rowspan="2" ><?=$dates['DS']?></td>
					</tr>
					<tr>
            <td><?=Yii::t('app','Stock in UzAutoMotors')?></td>
            <? $totalUamStock = 0; $totalByModel = [];?>
            <?foreach($models as $model){?>
              <td class="text-right"><?= number_format($data['uamstock'][$model->id],0,'',' ') ?? 0?></td>
              <?
								$totalUamStock += $data['uamstock'][$model->id];
								$totalByModel[$model->id] = $data['uamstock'][$model->id] ?? 0;
              ?>
            <?}?>
						<td class="text-right" ><?=number_format($totalUamStock,0,'',' ')?></td>
					</tr>
					<tr>
