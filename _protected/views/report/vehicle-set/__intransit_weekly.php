...
<tr>
      <td colspan="<?=count($period_weekly_veh) + 2?>" style="height: 30px;"><b><?=Yii::t('app', 'INTRANSIT VOLUME');?></b></td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    ...
    ?>
      ...
      <td></td>
      <? foreach ($period_weekly_veh as $col => $per) { ?>
        <? $qty = $this->context->getVehicleDataByDate($data_vehicle_intransit,$model->id,$per['from'],$per['to']);?>
