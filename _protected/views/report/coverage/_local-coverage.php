...
?>
<table class="table table-req" id="fix_table_l">
    <thead>
        ...
        <tr class="tr_minus_count">
            ...
            <div class="form-inline lf-div-filter" style="display: none;">

                <div class="form-group">
                    <select class="lf-filter-part-id cov-filter select2 form-control" style="width: 250px;"></select>
                </div>
        ...
        <? foreach ($data_local as $row) {
            ...
            <tr <?= ($i % 2 == 0) ? 'class="tr-item tr_odd"' : 'class="tr-item"' ?> data-part-id="<?= $row['part_id'] ?>" data-model-id="<?= $row['model_id'] ?>" data-unit-id="<?= $row['unit_id'] ?>">
                ...
                if ($row['totalstock'] > 0) {
                    ...
                    $total_doh_sum_l += round($doh_sum, 2);
                }
...
<div style="display: none">
	<p id="lf-parts"><?= json_encode($fparts) ?></p>
	<p id="lf-models"><?= json_encode($fmodels) ?></p>
	<p id="lf-units"><?= json_encode($funits) ?></p>
	<p id="lf-label-parts"><?= Yii::t('app', 'All parts'); ?></p>
	<p id="lf-label-models"><?= Yii::t('app', 'All models'); ?></p>
	<p id="lf-label-units"><?= Yii::t('app', 'All units'); ?></p>
</div>
...
$(document).ready(function() {
	...
	var parts = $.parseJSON($('#lf-parts').html());
	var models = $.parseJSON($('#lf-models').html());
	var units = $.parseJSON($('#lf-units').html());

	fillSelect($('.lf-filter-part-id'), parts, $('#lf-label-parts').html());
	fillSelect($('.lf-filter-model-id'), models, $('#lf-label-models').html());
	fillSelect($('.lf-filter-unit-id'), units, $('#lf-label-units').html());
	...
	$('.lf-filter-part-id').on('change', function () {
		$('.lf-btn-filter').trigger('click');
	});

	$('.lf-filter-model-id').on('change', function () {
		$('.lf-btn-filter').trigger('click');
	});

	$('.lf-filter-unit-id').on('change', function () {
		$('.lf-btn-filter').trigger('click');
	});
	// ***

	// Asosiy filter funksiyasi
	$('.lf-btn-filter').on('click', function () {

    var filter_part_id = $('.lf-filter-part-id').val();
		var filter_model_id = $('.lf-filter-model-id').val();
		var filter_unit_id = $('.lf-filter-unit-id').val();
		var ii = 0;
		var is_left = true;

		$('#fix_table_l tr.tr-item').each(function (i, row) {
			$(this).show(); 
		});
