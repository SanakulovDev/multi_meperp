...
use app\models\Part;
?>
<table class="table table-req" id="fix_table_c">
            ...
            <td colspan="12">
                ...
                <div class="form-inline cf-div-filter" style="display: none;">
                    ...
                    <div class="form-group">
                        <select class="cf-filter-part-id cov-filter select2 form-control" style="width: 250px;"></select>
                    </div>
    ...
    <tbody>
        ...
        <? foreach ($data_cons as $row) {
            ...
            <tr <?= ($i % 2 == 0) ? 'class="tr-item tr_odd"' : 'class="tr-item"' ?> data-part-id="<?= $row['part_id'] ?>" data-model-id="<?= $row['model_id'] ?>" data-unit-id="<?= $row['unit_id'] ?>">
                ...
                <td class="text-center td-nowrap" title="<?= $row['remark'] ?>"><?= $row['part_color'] ?></td>
                ...
                if ($row['totalstock'] > 0) {
                    ...
                    $total_doh_sum_c += round($doh_sum, 2);
                }
...
<div style="display: none">
	<p id="cf-parts"><?= json_encode($fparts) ?></p>
	<p id="cf-models"><?= json_encode($fmodels) ?></p>
	<p id="cf-units"><?= json_encode($funits) ?></p>
	<p id="cf-label-parts"><?= Yii::t('app', 'All parts'); ?></p>
	<p id="cf-label-models"><?= Yii::t('app', 'All models'); ?></p>
	<p id="cf-label-units"><?= Yii::t('app', 'All units'); ?></p>
</div>
...
$(document).ready(function() {
	...
	var parts = $.parseJSON($('#cf-parts').html());
	var models = $.parseJSON($('#cf-models').html());
	var units = $.parseJSON($('#cf-units').html());

	fillSelect($('.cf-filter-part-id'), parts, $('#cf-label-parts').html());
	fillSelect($('.cf-filter-model-id'), models, $('#cf-label-models').html());
	fillSelect($('.cf-filter-unit-id'), units, $('#cf-label-units').html());
	...
	$('.cf-filter-part-id').on('change', function () {
		$('.cf-btn-filter').trigger('click');
	});

	$('.cf-filter-model-id').on('change', function () {
		$('.cf-btn-filter').trigger('click');
	});

	$('.cf-filter-unit-id').on('change', function () {
		$('.cf-btn-filter').trigger('click');
	});
	// ***

	// Asosiy filter funksiyasi
	$('.cf-btn-filter').on('click', function () {

    var filter_part_id = $('.cf-filter-part-id').val();
		var filter_model_id = $('.cf-filter-model-id').val();
		var filter_unit_id = $('.cf-filter-unit-id').val();
		var ii = 0;
		var is_left = true;

		$('#fix_table_c tr.tr-item').each(function (i, row) {
			$(this).show(); 
		});
