<?php
    ...
    $this->title = Yii::t('app', 'Daily Requirement Short');
    ...
    <div class="panel">
        <div class="row">
              <div class="col-md-8 row" style="display: flex; flex-direction: row; justify-content:start; align-items:center;">
                ...
                <div class="col-md-2">
                  <a href="<?= \yii\helpers\Url::to(['report/daily-requirement-short', 'filter' => 1])?>" class="btn btn-success">Фильтр</a>
                </div>
                <div class="col-md-2">
                  <a href="<?= \yii\helpers\Url::to(['report/daily-requirement-short'])?>" class="btn btn-danger">Очистить фильтра</a>
                </div>
              </div>
      ...
      <table class="table table-req" id="fix_table_d">
        <thead style="    position: sticky;top: 0;">
                  <tr class="tr_head" style="background-color: #c7c0c0;">
                      ...
                      <th style="width: 100px;" class="text-center">1 День</th>
        ...
        <tbody>
            ...
            <?php foreach($data_daily as $row):?>
              ...
              <tr <?=($i%2 == 0) ? 'class="tr_odd"' : ''?>>
                  ...
                  <td style="text-align: center" class="current-month" data-part-id="<?= $row['part_id']?>" data-qty="<?=$row['current_day']?>">
                      <?= number_format($row['current_day'], 0, ',', ' ')?>
                  ...
                  <td class="balance" data-cash="<?=$row['currentDailyBalance']?>" style="text-align: center"><?php echo number_format($row['currentDailyBalance'], 0, ',', ' ') ?></td>
...
<?
    ...
    $(document).ready(function(){
      $('.requirement-part').on('change', function(e){
          ...
          let id = $(this).val();
          console.log(id);
          window.location.href = '/report/daily-requirement-short?part_id='+id;
      ...
      $('.current-month').on('click', function(){
        ...
        window.location.href= 'additional-daily-requirement-short?part_id='+$(this).data('part-id')+'&qty='+$(this).data('qty');
      })
