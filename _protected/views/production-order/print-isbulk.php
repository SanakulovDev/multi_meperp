<?php
	...
	/* @var $modelsToPrint app\models\ProductionOrder */
	if(isset($action) and $action != 'create-isbulk'){
...
<div id="print_qrcode_div">
	<? if(isset($modelsToPrint)){
		foreach($modelsToPrint as $model){
	    if($model['is_label'] ==1){
          echo $this->render('big-label', ['model' => $model]);
        }else{
		  echo $this->render('small-label', ['model' => $model]);
        }
		}
	} ?>
</div>
